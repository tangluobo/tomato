package com.tangluobo.tomato.ssh.zmodem.xfer.zm.proto;
...
public class Escape {
    ...
    public Escape(Action a) { this(a, 0); }
    public Escape(Action a, int l) { len = l; action = a; }
    ...
    public static Escape detect(byte b, boolean acceptsHeader) {
        ...
        if (r == null || ((!acceptsHeader) && r.action() == Action.HEADER)) return new Escape(Action.ESCAPE);
        return r;
    }
    ...
    public static boolean mustEscape(byte b, byte previous, boolean escapeCtl) {
        switch (b) {
            case 0xd:
            case (byte) 0x8d:
                if (escapeCtl && previous == '@') return true;
                break;
            case 0x18:
            case 0x10:
            case 0x11:
            ...
            case (byte) 0xff:
                return true;
            default:
                if (escapeCtl && ((b & 0x60) == 0)) return true;
        }
        return false;
    }

    public static byte escapeIt(byte b) {
        if (b == (byte) 0x7f) return ZModemCharacter.ZRUB0.value();
        if (b == (byte) 0xff) return ZModemCharacter.ZRUB1.value();
        if (b == (byte) ZModemCharacter.ZRUB0.value()) return 0x7f;
        if (b == (byte) ZModemCharacter.ZRUB1.value()) return (byte) 0xff;
        return (byte) (b ^ 0x40);
    }

    @Override
    public String toString() { return "Action=" + action + ", len=" + len; }
}
